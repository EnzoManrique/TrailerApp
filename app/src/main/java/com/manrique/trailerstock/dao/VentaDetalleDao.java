...
public interface VentaDetalleDao {
        ...
        // Obtener productos más rentables considerando precio de costo
        @Query("SELECT p.nombre as nombreProducto, " +
                        "SUM(vd.cantidad) as cantidadVendida, " +
                        "SUM(vd.cantidad * vd.precioUnitario) as totalVendido, " +
                        "SUM(vd.cantidad * p.precio_costo) as costoTotal, " +
                        "(SUM(vd.cantidad * vd.precioUnitario) - SUM(vd.cantidad * p.precio_costo)) as margenGanancia "
                        +
                        "FROM venta_detalles vd " +
                        "INNER JOIN productos p ON vd.productoId = p.id " +
                        "INNER JOIN ventas v ON vd.ventaId = v.id " +
                        ...
                        "LIMIT :limite")
        List<com.manrique.trailerstock.model.ProductoRentable> obtenerProductosMasRentables(long desde, int limite);
}
